    ...
    >
      ...
      <div className="aspect-video relative bg-muted overflow-hidden">
        ...
        <div className="absolute top-3 left-3 flex flex-wrap gap-2">
           <Badge variant="secondary" className="bg-black/50 text-white border-0 backdrop-blur-md text-xs font-normal">
      ...
      <div className="p-5">
        ...
        <div className="mt-4 flex items-center gap-1.5 overflow-hidden opacity-70 group-hover:opacity-100 transition-opacity">
           {template.slots.slice(0, 5).map((slot, i) => (
             <div key={i} className="h-1.5 flex-1 rounded-full bg-muted-foreground/30" />
           ))}
           {template.slots.length > 5 && (
             <span className="text-[10px] text-muted-foreground ml-1">+{template.slots.length - 5}</span>
           )}
        </div>
      </div>
    </div>
